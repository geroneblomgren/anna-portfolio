...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`about\` (
  	...
  	\`updated_at\` text,
  	\`created_at\` text,
  	FOREIGN KEY (\`photo_id_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`about_photo_id_idx\` ON \`about\` (\`photo_id_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`site_settings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`site_name\` text DEFAULT 'Anna Blomgren',
